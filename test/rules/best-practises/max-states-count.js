...
const {
  ...
  stateDef,
  constantDef,
} = require('../../common/contract-builder')
...
describe('Linter - max-states-count - V7 V8 additions', () => {
  it('should raise error when count of states too big', () => {
    const code = contractWithV7V8Header(stateDef(16))
  ...
  it('should not raise error for count of states that lower that max', () => {
    const code = contractWithV7V8Header([stateDef(10), constantDef(10)].join('\n'))
  ...
  it('should not raise error for count of states when it value increased in config', () => {
    const code = contractWithV7V8Header(stateDef(20))

    const report = linter.processStr(code, { rules: { 'max-states-count': ['error', 21] } })

    assertNoErrors(report)
  })

  it('should only count state declarations', () => {
    const code = contractWithV7V8Header(`
      uint private a;
      uint private b;
      uint private c;
    ...
    const report = linter.processStr(code, { rules: { 'max-states-count': ['error', 4] } })
