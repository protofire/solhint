...
function findSolhintBin() {
  ...
  try {
    ...
    // not found globally, fall back to npx
  }
  return 'npx solhint'
}
...
function runSolhint(target, cwd = process.cwd()) {
  ...
  const relTarget = path.isAbsolute(target) ? path.relative(cwd, target) : target

  const isNpx = bin === 'npx solhint'
  const cmd = isNpx ? ['npx', 'solhint', relTarget, '-f', 'json'] : [bin, relTarget, '-f', 'json']
...
function withTempSol(code, fn) {
  ...
  try {
    ...
    const cwdCfg = ['.solhint.json', '.solhintrc', '.solhintrc.json']
      ...
      .find(fs.existsSync)
    if (!cwdCfg) {
      fs.writeFileSync(cfg, JSON.stringify({ extends: 'solhint:recommended' }), 'utf8')
    }
    return fn(tmp, dir)
  } finally {
    try {
      ...
      // temp dir cleanup failure is non-fatal
    }
  }
}
...
function formatViolations(violations) {
  ...
  for (const item of violations) {
    ...
    lines.push(`  ${sev}  L${item.line}:${item.column}  \`${item.ruleId || '?'}\`  ${item.message}`)
...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        config: {
          type: 'object',
          description: 'Optional solhint config object (e.g. {"extends":"solhint:recommended"})',
        },
      },
      required: ['code'],
    },
...
function handleTool(name, args) {
  switch (name) {
    ...
    case 'lint_file': {
      ...
      const resolvedPath = path.isAbsolute(filePath) ? filePath : path.join(resolvedCwd, filePath)
    ...
    case 'explain_rule': {
      const { ruleId } = args
      if (!ruleId) return { content: 'ruleId is required' }
      return { content: explainRule(ruleId) }
    }
