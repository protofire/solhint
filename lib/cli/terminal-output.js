...
function printMcpHint(linksSupported, env = process.env, stream = process.stdout) {
  const isCi = env.CI && env.CI !== 'false' && env.CI !== '0'
  if (!stream.isTTY || isCi) return

  const mcpUrl = 'https://github.com/protofire/solhint/tree/develop/packages/mcp-server'
  const cmd = 'claude mcp add solhint npx @solhint/mcp-server'
  const hint = linksSupported
    ? `  💡 Claude Code user? ${clickableLink(mcpUrl, chalk.cyan.bold(cmd))}`
    : `  💡 Claude Code user? ${chalk.cyan.bold(cmd)}`
...
function printPoster(issuesFound = true, env = process.env, stream = process.stdout) {
  ...
  printMcpHint(linksSupported, env, stream)
}
