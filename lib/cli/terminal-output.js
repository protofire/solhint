...
function isInteractiveTerminal(env = process.env, stream = process.stdout) {
  const isCi = env.CI && env.CI !== 'false' && env.CI !== '0'

  return Boolean(stream.isTTY) && !isCi
}

function checkForUpdate(env = process.env, stream = process.stdout) {
  // The update notice is written for a person reading a terminal. When the output is
  // piped or the run happens in CI nobody acts on it, and the registry request adds
  // seconds to every invocation.
  if (!isInteractiveTerminal(env, stream)) return Promise.resolve()
...
// A missing or unparseable version means "unknown", not "too old": TERM_PROGRAM often
// survives ssh/sudo/wrappers while TERM_PROGRAM_VERSION does not.
function parseTerminalVersion(rawVersion) {
  const [major, minor] = String(rawVersion || '')
    .split('.')
    .map((part) => Number.parseInt(part, 10))

  return { major: major || 0, minor: minor || 0, known: Number.isInteger(major) }
}
...
function supportsTerminalHyperlinks(env = process.env, stream = process.stdout) {
  ...
  // OSC 8 does not reach the outer terminal from inside tmux/screen unless passthrough is on
  if (/^(screen|tmux)/.test(env.TERM || '')) return false
  // iTerm2 forwards LC_TERMINAL over ssh, where TERM_PROGRAM is lost
  if (env.LC_TERMINAL === 'iTerm2') return true

  const { major, minor, known } = parseTerminalVersion(env.TERM_PROGRAM_VERSION)

  if (env.TERM_PROGRAM === 'iTerm.app') {
    return !known || major > 3 || (major === 3 && minor >= 1)
  }

  if (env.TERM_PROGRAM === 'vscode') {
    return !known || major > 1 || (major === 1 && minor >= 72)
  }

  if (['ghostty', 'Hyper', 'WezTerm', 'WarpTerminal', 'zed'].includes(env.TERM_PROGRAM)) {
    return true
  }

  if (['alacritty', 'xterm-kitty', 'xterm-ghostty'].includes(env.TERM)) return true
...
function clickableLink(url, text) {
  ...
  return `\u001B]8;;${url}${stringTerminator}${text}\u001B]8;;${stringTerminator}`
}
...
function printSuccessPoster(stream = process.stdout, env = process.env) {
  if (!isInteractiveTerminal(env, stream)) return false
