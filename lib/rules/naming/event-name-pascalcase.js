...
const ruleId = 'event-name-pascalcase'
const meta = {
  type: 'naming',

  docs: {
    description: 'Event name must be in PascalCase.',
...
class EventNamePascalCaseChecker extends BaseChecker {
  ...
  EventDefinition(node) {
    if (naming.isNotPascalCase(node.name)) {
      this.error(node, 'Event name must be in PascalCase', this.fixStatement(node))
    }
  }
}

module.exports = EventNamePascalCaseChecker
