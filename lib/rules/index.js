...
const notifyRuleDoesntExist = _.memoize((ruleId) => {
  console.warn(chalk.yellow(`\n ===>>>> [solhint] Warning: Rule '${ruleId}' doesn't exist`))
})
...
function loadPlugin(pluginName, { reporter, config, inputSrc, fileName }) {
  let plugins
  const id = `solhint-plugin-${pluginName}`

  try {
    let extraPluginPaths = []

    if (Array.isArray(config.pluginPaths)) {
      extraPluginPaths = config.pluginPaths
    } else if (config.pluginPaths) {
      extraPluginPaths = [config.pluginPaths]
    }
    ...
    const resolutionPaths = [
      process.cwd(),
      ...extraPluginPaths,
      ...extraPluginPaths.map((p) => path.join(p, 'node_modules')),
    ]

    const resolved = require.resolve(id, { paths: resolutionPaths })
    plugins = require(resolved)

    // support ESM default export
    if (!Array.isArray(plugins) && Array.isArray(plugins?.default)) {
      plugins = plugins.default
    }
  } catch (e) {
    const reason = e && e.message ? ` Reason: ${e.message}` : ''
    console.warn(chalk.yellow(`[solhint] Warning: Could not load ${id}, ignoring it.${reason}`))
    return []
  }

  if (!Array.isArray(plugins)) {
    console.warn(
      chalk.yellow(`[solhint] Warning: Plugin ${id} doesn't export an array of rules. Ignoring it.`)
    )
    return []
  }

  const instances = plugins.map((Plugin) => new Plugin(reporter, config, inputSrc, fileName))
