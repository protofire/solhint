...
const meta = {
  ...
  fixable: false,
  recommended: true,
  defaultSetup: [DEFAULT_SEVERITY, { searchOn: ['/node_modules', '/lib'], includeDefaults: true }],
...
class ImportPathChecker extends BaseChecker {
  constructor(reporter, config, fileName) {
    ...
    this.searchOn = config ? config.getObjectPropertyArray(ruleId, 'searchOn', 1, '') : ''

    const includeDefaults = config
      ? config.getObjectPropertyBoolean(ruleId, 'includeDefaults', 2, true)
      : true
