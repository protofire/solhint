...
const meta = {
  ...
  docs: {
    ...
    notes: [
      ...
      {
        note: '`searchOn` will concatenate with `default path locations` to check',
      },
      {
        note: [
          '*Default Locations:*',
  ...
  fixable: false,
  recommended: true,
  // defaultSetup: [DEFAULT_SEVERITY, ['/node_modules']],
...
/*
    rules: { 'import-path-check': [ 'error', { searchOn: ['/project/node_modules'], includeDefaults: false } ] }

  
  - If searchOn is empty, includeDefaults is ignored and only DEFAULT_LOCATIONS are used.
  - If searchOn has values and includeDefaults is false, only searchOn is used.
  - If searchOn has values and includeDefaults is true, both searchOn and DEFAULT_LOCATIONS are used together.

  - Test fails when there's extends: 'solhint:all/recommended', and rule is ALSO specified with an empty 
    searchOn and includeDefaults is false because searchOn WILL HAVE default element
*/
...
class ImportPathChecker extends BaseChecker {
  constructor(reporter, config, fileName) {
    ...
    // console.log('config :>> ', config ? config.rules['import-path-check'][1] : '')

    this.searchOn = config ? config.getArray(ruleId) : []

    if (this.searchOn.length === 0 || this.searchOn[0] === LOCATION_REPLACEMENT_FLAG) {
      this.searchOn = [...DEFAULT_LOCATIONS]
    } else {
      this.searchOn = this.searchOn.concat(DEFAULT_LOCATIONS)
    }
