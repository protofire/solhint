...
class BlockValidator extends KnowLineValidator {
    ...
    validateIndentOfNestedElements(block) {
        const requiredIndent = correctIndentOf(this._alreadyVerifiedRootNode(block.ctx)) + this.indent;
    ...
    validateEndBracketIndent(block) {
        const endBracketCorrectIndent = correctIndentOf(this._alreadyVerifiedRootNode(block.ctx));
    ...
    _alreadyVerifiedRootNode(ctx) {
        const rootNode = firstNodeOfLine(ctx);

        // in case when open bracket is the first on the line
        if (rootNode === ctx && typeOf(ctx) === 'block' && this._firstChildText(ctx) === '{') {
            const parent = ctx.parentCtx;

            if (typeOf(parent) === 'functionDefinition') {
