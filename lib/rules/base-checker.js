...
class BaseChecker {
  static ajv = new Ajv({ allErrors: true, jsonPointers: true });
  ...
  constructor(reporter, ruleId, meta) {
    ...
    const schemaKey = ruleId + JSON.stringify(meta.schema || {});
    if (!BaseChecker.validators.has(schemaKey)) {
      const schema = buildSchema();
      BaseChecker.validators.set(schemaKey, BaseChecker.ajv.compile(schema));
    }
