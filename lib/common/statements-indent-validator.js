...
function hasNoSpaceAfter(ctx) {
    ...
    return noSpaces(_nextToken, token) || !onSameLine(token, _nextToken);
}
...
function hasNoSpacesBefore(ctx) {
    ...
    return noSpaces(token, _prevToken) || !onSameLine(token, _prevToken);
}
...
function hasSpaceBefore(ctx) {
    ...
    return hasSpace(token, _prevToken) || !onSameLine(token, _prevToken);
}
...
function hasSpaceAfter(ctx) {
    ...
    return hasSpace(_nextToken, token) || !onSameLine(token, _nextToken);
}
...
function spaceDiff(token1, token2) {
    return token1.start - token2.stop;
}


function noSpaces(token1, token2) {
    return spaceDiff(token1, token2) === 1;
}


function hasSpace(token1, token2) {
    return spaceDiff(token1, token2) === 2;
}
