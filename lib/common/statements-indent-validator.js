...
class NoSpacesAround extends Term {
    ...
    listenError (callback) {
        const ctx = this.ctx;
        const tokenStart = ctx.start || ctx.symbol;
        const tokenStop = ctx.stop || ctx.symbol;
        const _nextToken = nextToken(ctx);
        const _prevToken = prevToken(ctx);

        if (!(_nextToken.start - tokenStop.stop === 1 && tokenStart.start - _prevToken.stop === 1)) {
...
class SpaceAroundOrNot extends Term {
    ...
    listenError (callback) {
        ...
        const hasNoSpacesAround = _nextToken.start - tokenStop.stop === 1 && tokenStart.start - _prevToken.stop === 1;
        const hasSpacesAround = _nextToken.start - tokenStop.stop === 2 && tokenStart.start - _prevToken.stop === 2;
